var UI;
(function (UI) {
    var CSSImportURL = utils.CSSImportURL;
    ...
    class UWPSpinner extends HTMLElement {
        spinner;
        static observedAttributes = ["running", "size"];
        constructor() {
            super();
        ...
        get running() {
            const value = this.getAttribute("running");
    ...
    class CascadingMenu extends HTMLElement {
        arrow;
        menuItem;
        contextmenu;
        constructor() {
            ...
            this.menuItem.addEventListener("mouseenter", () => {
                const menuContent = this.contextmenu.assignedElements()[0];
            ...
            const _this = this;
            function handler() {
                const menuContent = _this.contextmenu.assignedElements()[0];
    ...
    UI.CascadingMenu = CascadingMenu;
    ...
    class MenuBuilderNormalItem {
        ...
        get icon() {
            return this.element.querySelector("div.icon_img[data-type]").dataset.type;
        }
        set label(label) {
        ...
        set icon(icon) {
            this.element.querySelector("div.icon_img[data-type]").dataset.type = icon;
        }
    ...
    UI.MenuBuilderNormalItem = MenuBuilderNormalItem;
    class MenuBuilderSeparator {
        element;
        type = "separator";
        constructor() {
    ...
    UI.MenuBuilderSeparator = MenuBuilderSeparator;
    class MenuBuilderCascadingMenu {
        element;
        type = "cascading-menu";
        constructor(label, menu, icon) {
            this.element = document.createElement("cascading-menu");
            this.element.innerHTML = `
            <div class="icon_img" data-type="${icon || "none"}"></div>
            <span>${label}</span>
            `;
            const contextmenu = menu instanceof ContextMenu ? menu : menu.build();
            contextmenu.slot = "contextmenu";
            this.element.appendChild(contextmenu);
        }
    }
    UI.MenuBuilderCascadingMenu = MenuBuilderCascadingMenu;
    // noinspection JSUnusedGlobalSymbols
    ...
    UI.ContextMenuBuilder = ContextMenuBuilder;
    function attachContextMenu(item, contextMenu) {
    ...
    UI.attachContextMenu = attachContextMenu;
    function removeContextMenu(item) {
        item.dataset.menuAttached = undefined;
    }
    UI.removeContextMenu = removeContextMenu;
